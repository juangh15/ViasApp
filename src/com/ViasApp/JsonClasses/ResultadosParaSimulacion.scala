  ...
  ManejoJson.escribirArchivo("C:\\Users\\prueba\\Desktop\\ViasApp-master\\src\\com\\ViasApp\\resultados.json",
    new InputResultadosSimulacion(new ResultadosSimulacion(
      ...
      new MallaVial(vias,
                    ...
                    velocidadMaxima,
                    longitudPromedio,
                    new VehiculosEnInterseccion(promedioOrigen,
                                                promedioDestino,
                                                sinOrigen,
                                                sinDestino)),
      new Tiempos(simulacion,
                  realidad),
      new Velocidades(minima,
                      maxima,promedio),
      new Distancias(minima2,
                     maxima2,
                     promedio2),
      new Comparendos(cantidad,promedioPorcentajeExceso)
      )
    )
  )
}